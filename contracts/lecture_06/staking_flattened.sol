...
// File: contracts/lecture_06/staking.sol
...
contract staking is Hassan{
    ...
    function stake(uint256 stakeValue) public {
        ...
        require((block.timestamp - block.timestamp % 1) >= stakingTime, "You can only stake once per day");
    ...
    function unStake() public{
        ...
        require((block.timestamp - block.timestamp % 100)>= stakingTime, "You can only un-stake after 100 seconds");
        stakingCalculation(stakeVal);
        stakingToken.transferFrom(address(this), msg.sender, stakeVal);

    }

    function stakingCalculation(uint256 stakeValue) public view returns (uint){
        uint totalValue = stakeValue * (stakingTime/100);
        return totalValue;
    }
